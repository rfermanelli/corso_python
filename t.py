...
# from dto.studente_dto import StudenteDTO
# from service.studente_service import StudenteService
# from datetime import datetime

# La funzione riceve in ingresso il valore della scelta effettuata dall'utente
# e chiama la funzione DAO corrispondente.
def funzione_di_scelta(funzione_scelta):

    if funzione_scelta == "I":
        input_matricola = int(input("Inserisci la matricola dello studente: "))
        input_nome= input("Inserisci il nome dello studente: ")
        input_cognome= input("Inserisci il cognome dello studente: ")
        input_dob = input("Inserire data di nascita secondo il seguente formato:(yyyy-mm-dd): ")
        input_cf = input("Inserire codice fiscale: ")
        studente = StudenteDTO(input_matricola, input_nome,input_cognome, input_dob,input_cf)
        insert_studente = StudenteService.insert_studente(input_matricola, input_nome,input_cognome, input_dob,input_cf)
        return insert_studente

    if funzione_scelta == "S":
...
while True:
    try:
        ...
        assert funzione_scelta.upper() in ("I", "U", "D", "S", "A")
        break
